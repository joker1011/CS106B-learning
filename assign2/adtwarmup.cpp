...
//**********STUDENT_TEST*****************
STUDENT_TEST("duplicateNegatives, q = {-7}") {
    Queue<int> q = {-7};
    Queue<int> expected = {-7, -7};
    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}
STUDENT_TEST("duplicateNegatives, input has one negative on the front") {
    Queue<int> q = {-7,8,9,-5};
    Queue<int> expected = {-7,-7,8,9,-5,-5};
    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}
STUDENT_TEST("duplicateNegatives, input has one negative on the back") {
    Queue<int> q = {-9,-7,-5};
    Queue<int> expected = {-9,-9,-7,-7,-5,-5};
    duplicateNegatives(q);
    EXPECT_EQUAL(q, expected);
}
